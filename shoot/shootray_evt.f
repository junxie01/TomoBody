...
25    format(a5,1x,2(f11.6,1x),2(f9.4,1x),f6.1,2f8.2,f7.1,f7.2,f8.2,f9.4,1x,f9.5,f10.3)
36    format(f10.3,2f10.4,f10.3,f10.3,f10.4,f8.2)
...
c      rpd=pi/180.d0
      ...
      do 2100 ievt=1,nevt
         ...
         write(6,'(a,2i6,4x,a40)')"ievt,nobs:",ievt,nobs(ievt),header(ievt)
...
c euler_rot: calculate rotation matrix from finding cos(x-prime,x)
         call euler_rot(radsrc,sth,sph,rsurf,rth,rph,g2e,e2g)
c      write(6,*)'backazimuth and epicentral distance=',azsrdeg,distrs
       write(6,'(a,i8,2f6.1)')'    ray bkaz epidist:',iobs,azsrdeg,distrs
...
c points
       ...
       write(17,25)stnnm(iobs),rlat(iobs),rlon(iobs),slat(iobs),slon(iobs),
     &  sdep(iobs),blat(iobs),blon(iobs),bdep(iobs),
     &  del(iobs),azs(iobs),ctim(iobs),s2n(iobs),ptim(iobs)
       do i=1,ni
